...
本文先后xxxx。另：熔断和断路同一概念下的不同表达，熔断常与限流、路由等微服务策略并提，而断路（器）可以理解为熔断策略的具体实现。

## 前言

![](http://ww1.sinaimg.cn/large/bf8f1836ly1g2amun40fnj218g0x0e25.jpg)

*这是关于熔断的两部分系列文章的第二部分。在[第一部分](https://glasnostic.com/blog/preventing-systems-fail-circuit - breaking-part1)中，我们介绍了该模式以及开发人员和操作人员如何以不同的方式处理它。而在本文中，我们将探讨它的典型用例以及如何在现代服务中间件中实现它。*
...
Hystrix是一个Java库，开发人员可以使用它用熔断逻辑封装服务调用。它基于阈值，可以立即执行失败调用并执行回滚逻辑，如图1所示(https://glasnostic.com/blog/preventing-systems-fail-circuit -breaking-part-1#figure-1)。除了提供超时和并发限制之外，它还可以向监视工具发布度量。最后，当与[Archaius](https://github.com/Netflix/archaius)库一起使用时，它还可以支持动态配置更改。
...
例如，[figure 3](https://glasnostic.com/blog/preventing-systems-failing-ing-breaking-part- 2#figure-3)显示了一个通道（channel）的设置，该通道用于监视和控制一组互不相关的服务中间歇重复出现的延迟峰值。在不寻找假定的出错根源的情况下，运维人员决定首先通过中断更长时间运行的请求来控制这种情况。他们首先定义一个覆盖有问题的服务和任何潜在客户端的新通道，然后对通道控制的交互施加适当的延迟限制，从而实现这一点。这使得运营团队能够在发生问题时控制局面，直到工程师能够提供一个修复方案。
...
当然，最初的政策往往只是首先尝试补救局面，并且需要对调整保持开放的态度。调整或补充开放政策既快又容易。例如，运维团队可能会发现，初始通道策略可以通过第一个断开非关键任务客户机的电路来进一步细化，从而使关键任务客户机尽可能长时间地不受影响。为了实现这一点，他们可以定义一个只覆盖非关键任务客户端的细化通道，并添加一个基于连接和请求许可中断它们的策略。[Figure 4](https://glasnostic.com/blog/preventing-systemic-failure-circuit-break -part-2# Figure -4)显示了这样一个辅助细化通道，在原始延迟断路器跳闸之前，通过设置的并发性和请求策略切断链路中非关键的客户端，从而提高了关键任务系统的可用性。
...
作为一种开发人员模式，它主要用作一种相当基本的补偿策略，如果不考虑每个特定的调用，就很难对其进行细化。另一方面，断路器作为一种操作模式，旨在减轻系统的压力，以管理系统的稳定性和性能。通过将其与其他稳定模式(如超时或背压)相结合，通常可以进一步改进其行为。操作断路器过去依赖于单独部署的服务中间件，如API网关或服务网格。然而，由于服务网格主要解决了开发人员的关注点，所以作为一种操作模式对断路的支持是有限的，而且在实现之间是不一致的。因此，最好使用像Glasnostic这样的云流量控制器来断开操作电路。

Glasnostic是一个云流量控制器，它使操作团队能够控制他们的有机架构所展示的复杂的紧急行为。这种公开化的控制平面允许操作人员将许多操作模式(如断路)应用到任意一组交互中。它还允许操作人员通过组合模式来细化操作目标。这使得操作人员和开发人员可以更快地部署到生产环境中，并支持业务以适应不断变化的业务需求。
